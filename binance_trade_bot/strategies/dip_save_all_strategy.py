...
class Strategy(AutoTrader):
    def initialize(self):
        ...
        self.high_point_sell_signal_check = -2.5
        self.low_point_buy_signal_check = 0.1
        self.high_point_buy_signal_check = 0.2
    ...
    def scout(self):
        ...
        for coin in self.config.SUPPORTED_COIN_LIST:
            ...
            low_point_price = price_history[self.low_point_tick]
            high_point_price = price_history[self.high_point_tick]
            low_point_velocity = (coin_price - low_point_price) / low_point_price * 100
            high_point_velocity = (coin_price - high_point_price) / high_point_price * 100
            low_velocities.append(low_point_velocity)
            high_velocities.append(high_point_velocity)

        low_point_velocity = statistics.mean(low_velocities)
        high_point_velocity = statistics.mean(high_velocities)
        ...
        if self.is_in_save_mode != True and self.hits >= 50:
